...
class Decode(nn.Module):
    def __init__(self, device1, device2):
        ...
        self.h = nn.ModuleList([DecodeBlock(self.config, self.Param, layer_idx=i, device=self.device) for i in range(self.config.num_hidden_layers)])
...
class DecodeBlock(nn.Module):
    def __init__(self, config, modelParameters, layer_idx, device):
        ...
        self.attn = DecodeAttention(config, modelParameters, layer_idx=layer_idx, device=self.device)
