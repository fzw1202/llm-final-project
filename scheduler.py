...
def add_new_ids(tokenizer, input_ids, attention_mask, new_input_ids, device):
    new_id_column = torch.full(
        (input_ids.size(0), 1), tokenizer.pad_token_id, device=device
    )
...
class Scheduler:
    ...
    def encode(self, prompts, seq_len):
        ...
        attention_mask = torch.cat(
            (
                attention_mask,
                torch.full(
                    (attention_mask.size(0), padding_len), 0, device=self.getDevice()
                ),
            ),
            dim=-1,
        )
    ...
    def kvcachePadding(self, kv, seq_len):
        ...
        key_padding = torch.full(
            (batch_size, padding_len, embed_dim), 0, device=self.getDevice()
        )
        value_padding = torch.full(
            (batch_size, padding_len, embed_dim), 0, device=self.getDevice()
        )
        return [
            (
                torch.cat((kv_item[0], key_padding), dim=1),
                torch.cat((kv_item[1], value_padding), dim=1),
            )
            for kv_item in kv
        ]
    ...
    def encodePrompts(self, prompts):
        ...
        return encodeBatch["input_ids"].to(self.getDevice()), encodeBatch[
            "attention_mask"
        ].to(self.getDevice())
    ...
    def continuous_batch(self, first_batch_prompts, left_prompts):
        ...
        with torch.no_grad():
            ...
            input_ids, mask = add_new_ids(
                self.tokenizer, input_ids, mask, new_ids, device=self.getDevice()
            )
            generate_result = []
        ...
        while input_ids.size(0) > 0:
            with torch.no_grad():
                ...
                stop_decode_indexs = [
                    index
                    for index, (new_id, mask) in enumerate(zip(new_ids, mask))
                    if new_id in self.stop_words_token
                    or mask.sum() > self.max_docode_token_num
                ]
                ...
                for index in stop_decode_indexs:
                    ...
                    input_ids = torch.cat(
                        (input_ids[:fix_index], input_ids[fix_index + 1 :]), dim=0
                    )
                    mask = torch.cat((mask[:fix_index], mask[fix_index + 1 :]), dim=0)
                    kv = [
                        (
                            torch.cat(
                                (kv_item[0][:fix_index], kv_item[0][fix_index + 1 :]),
                                dim=0,
                            ),
                            torch.cat(
                                (kv_item[1][:fix_index], kv_item[1][fix_index + 1 :]),
                                dim=0,
                            ),
                        )
                        for kv_item in kv
                ...
                if len(left_prompts) > 0 and len(stop_decode_indexs) > 0:
                    ...
                    prefill_ids, prefill_mask = self.encodePrompts(
                        left_prompts[:prefill_num]
                    )
                    ...
                    if seq_len < input_ids.size(1):
                        ...
                        kv = [
                            (kv_item[0][:, :-reduced_len], kv_item[1][:, :-reduced_len])
                            for kv_item in kv
                        ]

                    if seq_len > prefill_ids.size(1):
                        prefill_ids, prefill_mask = self.encode(
                            left_prompts[:prefill_num], seq_len - 1
                        )
                        left_prompts = left_prompts[prefill_num:]
                        new_prefill_ids, new_prefill_kv = self.prefillModel(
                            prefill_ids, prefill_mask
                        )
                        new_ids, new_mask = add_new_ids(
                            self.tokenizer,
                            prefill_ids,
                            prefill_mask,
                            new_prefill_ids,
                        ...
                        kv = [
                            (
                                torch.cat((old_kv[0], new_kv[0]), dim=0),
