...
class Prefill(nn.Module):
    def __init__(self, device1, device2):
        ...
        self.Param = torch.load(BIN_PATH, map_location=torch.device(self.device), weights_only=True)
        ...
        self.h = nn.ModuleList([PrefillBlock(self.config, self.Param, layer_idx=i, device=self.device) for i in range(self.config.num_hidden_layers)])
...
class PrefillBlock(nn.Module):
    def __init__(self, config, modelParameters, layer_idx, device):
        ...
        self.attn = PrefillAttention(config, modelParameters, layer_idx=self.layer_idx, device=self.device)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon, device=self.device)
        self.mlp = MLP(inner_dim, config, layer_idx, modelParameters, device=self.device)
...
class PrefillAttention(nn.Module):
    def __init__(self, config, modelParameters, layer_idx, device):
        ...
        self.attn.bias = nn.Parameter(self.Param[attn_bias_ParaName].squeeze(0).squeeze(0))
